...

{
    ...
    function appendSlides() {
      var numSlideSets = numSlides / 4;
      var classCount = 1;

      for (var x = 0; x < numSlideSets; x++) {
        categoryCount = 1;
        for (var i = 0; i < 4; i++) {

          picNum = getRandomInt(1, 1);
          setSlideCategory();

          //setPicNum();

          if(classCount === 1){
            $( ".slides--images" ).append( '<div class="slide slide--current"><div class="slide__img slide__img_' + classCount + '" style="background-image: url(img/' + category + '_' + picNum + '.png)"></div></div>' );
            $( ".slides--titles" ).append( '<div class="slide slide--current neon-text"><h2 class="slide__title slide__title_' + classCount + '">' + category + '</h2></div>' );

          }else{
            $( ".slides--images" ).append( '<div class="slide"><div class="slide__img slide__img_' + classCount + '" style="background-image: url(img/' + category + '_' + picNum + '.png)"></div></div>' );
            $( ".slides--titles" ).append( '<div class="slide neon-text"><h2 class="slide__title slide__title_' + classCount + '">' + category + '</h2></div>' );
          }
          categoryCount++;
          classCount++;
        }
    ...
    function setPicNum(){
      if(addedSlides.includes(category + '_' + picNum)){
        picNum = getRandomInt(1, 1);
        setPicNum();
      }else{
        addedSlides.push(category + '_' + picNum);
      }
    }
    ...
    class Slideshow {
        ...
        createFrame() {
            ...
            this.DOM.svg.innerHTML = `
                ...
                <image xlink:href="img/background.png" clip-path="url(#shape__clip)" x="0" y="0" width="${imgFillSize.width}px" height="${imgFillSize.height}px"/>
            `;
